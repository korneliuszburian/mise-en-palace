...
Current task state: the scoped CLI test typecheck gate now includes doctor
readiness/check tests. This does not prove full CLI test typecheck; it widens
the clean subset without runtime or assertion changes.

Latest slice:

```txt
CLI doctor test typecheck widening slice.
Report:
