import { describe, expect, it } from "vitest";

import { formatMaintenanceQueueSmokeReportLines } from "../internal/smoke/maintenance-queue-smoke.js";

describe("maintenance queue smoke report formatting", () => {
  it("prints the transition and cleanup proof lines", () => {
    expect(
      formatMaintenanceQueueSmokeReportLines({
        writeBoundaryValidatedCount: 6,
        enqueuedRecordCount: 6,
        queuedReadbackCount: 6,
        claimedRecordCount: 7,
        recoveredRecordCount: 1,
        candidateConcurrentRunCount: 2,
        candidateReplayRunCount: 1,
        candidatePersistedCount: 1,
        candidateStableId: true,
        successRecordedCount: 2,
        skipRecordedCount: 1,
        retryRecordedCount: 1,
        deadLetterRecordedCount: 2,
        cleanupDeletedCount: 6,
        candidateCleanupDeletedCount: 1,
        remainingMarkerCount: 0,
        cleanedUp: true
      })
    ).toEqual([
      "Maintenance write-boundary records validated: 6",
      "Maintenance queue records enqueued: 6",
      "Queued records read back: 6",
      "Queue record claims persisted: 7",
      "Stale running records recovered: 1",
      "Concurrent maintenance candidate runs: 2",
      "Replay maintenance candidate runs: 1",
      "Persisted semantic maintenance candidates: 1",
      "Maintenance candidate id stable: yes",
      "Queue success records persisted: 2",
      "Queue skip records persisted: 1",
      "Queue retry records persisted: 1",
      "Queue dead-letter records persisted: 2",
      "Cleanup deleted queue records: 6",
      "Cleanup deleted candidate projects: 1",
      "Cleanup remaining marker count: 0",
      "Cleanup: completed",
      "Maintenance queue smoke: passed"
    ]);
  });
});
