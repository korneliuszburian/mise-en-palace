...
const runActivationSmokeTarget: DbSmokeTargetHandler = async (
  ...
  return smokeResultFromCleanup(
    ...
    [
      ...
      `Temporal governing source claim: ${report.temporalGoverningSourceClaimId}`,
      `Temporal ranked-down source claim: ${report.temporalRankedDownSourceClaimId}`,
      `Temporal boundary source claim: ${report.temporalBoundarySourceClaimId}`,
      `Temporal dissenting peer source claim: ${report.temporalDissentingPeerSourceClaimId}`,
      `Current source claim edge: ${report.currentSourceClaimEdgeId}`,
      `Expired source claim edge: ${report.expiredSourceClaimEdgeId}`,
      `Equal-boundary source claim edge: ${report.equalSourceClaimEdgeId}`,
      `Equal-boundary dissent source claim edge: ${report.expiredDissentSourceClaimEdgeId}`,
      `DecisionPacket temporal source claims: ${report.temporalPacketSourceClaimIds.join(", ")}`,
      `DecisionPacket brief temporal source claims: ${report.temporalPacketBriefSourceClaimIds.join(", ")}`,
      `DecisionPacket temporal SourceDecision edges: ${report.temporalPacketSourceDecisionEdgeIds.join(", ")}`,
      `Persisted temporal rank-down edges: ${report.temporalPersistedRankDownEdgeIds.join(", ")}`,
      `Persisted temporal rank-down governing claims: ${report.temporalPersistedRankDownGoverningSourceClaimIds.join(", ")}`,
      `Graph-expanded endpoint lexical seeds: ${report.expandedEndpointLexicalSeedCount}`,
      `DecisionPacket graph-expanded exclusions: ${report.expandedEndpointPacketExclusionCount}`,
      `DecisionPacket graph-expanded superseded paths: ${report.expandedEndpointSupersededPathCount}`,
      `DecisionPacket graph-expanded governing refs: ${report.expandedEndpointGoverningRefCount}`,
      `DecisionPacket current edge refs: ${report.currentSourceClaimEdgePacketRefCount}`,
      `DecisionPacket expired edge refs: ${report.expiredSourceClaimEdgePacketRefCount}`,
      `DecisionPacket equal-boundary edge refs: ${report.equalSourceClaimEdgePacketRefCount}`,
      `DecisionPacket equal-boundary dissent edge refs: ${report.expiredDissentSourceClaimEdgePacketRefCount}`,
      ...
      `Relevant memory included decisions: ${report.relevantMemoryIncludedDecisionCount}`,
      `One-term distractor candidates: ${report.relevanceDistractorCandidateCount}`,
      `One-term distractor excluded decisions: ${report.relevanceDistractorExcludedDecisionCount}`,
      `One-term distractor context inclusions: ${report.relevanceDistractorContextInclusionCount}`,
      `DecisionPacket memory refs: ${report.decisionPacketMemoryRefCount}`,
      `DecisionPacket selected relevant memory: ${report.decisionPacketRelevantMemorySelected ? "yes" : "no"}`,
      `DecisionPacket distractor refs: ${report.decisionPacketDistractorRefCount}`,
      `DecisionPacket persisted readback: ${report.decisionPacketReadbackMatched ? "matched" : "mismatch"}`,
      `Relevant memory canonical score: ${report.relevantMemoryCanonicalScore}`,
      `Relevant memory persisted score: ${report.relevantMemoryPersistedScore}`,
      `Expired projection candidate refs: ${report.staleProjectionCandidateRefCount}`,
      `Expired projection decision refs: ${report.staleProjectionDecisionRefCount}`,
      `Expired projection context refs: ${report.staleProjectionContextRefCount}`,
      `Expired projection packet refs: ${report.staleProjectionPacketRefCount}`,
      `Expired projection historical score: ${report.staleProjectionHistoricalScore}`,
...
const formatDecisionPacketReturnLoopSmokeLines = (
  ...
  `Matching SourceDecision: ${report.matchingSourceDecisionId}`,
  `Matching SourceDecision feedback outcome: ${report.matchingSourceDecisionFeedbackOutcome}`,
  `Matching SourceDecision application count: ${report.matchingSourceDecisionApplicationCount}`,
  `Matching SourceDecision helped outcome count: ${report.matchingSourceDecisionHelpedOutcomeCount}`,
  ...
  `Mismatched feedback stayed out of issued packet: ${yesNo(report.mismatchedFeedbackStayedOutOfIssuedPacket)}`,
  `Issued packet governing decisions: ${report.issuedPacketGoverningDecisionIds.join(", ")}`,
  `Issued packet SourceDecision ids: ${report.issuedPacketSourceDecisionIds.join(", ")}`,
  `Issued packet stale decisions: ${report.issuedPacketStaleDecisionIds.join(", ")}`,
  `Issued packet caveated source claims: ${report.issuedPacketCaveatedSourceClaimIds.join(", ")}`,
  `Issued packet retains activated decisions: ${yesNo(report.issuedPacketRetainsActivatedDecision)}`,
  `Issued packet identity retained: ${yesNo(report.issuedPacketIdentityRetained)}`,
  ...
  `Standalone anti-memory proof run: ${report.standaloneAntiMemoryProofRunId}`,
  `Standalone anti-memory record: ${report.standaloneAntiMemoryRecordId}`,
  `Standalone anti-memory retrieval run: ${report.standaloneAntiMemoryRetrievalRunId}`,
  `Standalone anti-memory candidate count: ${report.standaloneAntiMemoryCandidateCount}`,
  `Standalone anti-memory excluded decision count: ${report.standaloneAntiMemoryExcludedDecisionCount}`,
  `Standalone anti-memory context exclusion count: ${report.standaloneAntiMemoryContextExclusionCount}`,
  `Standalone anti-memory packet rejected paths: ${report.standaloneAntiMemoryPacketRejectedPathIds.join(", ")}`,
  `Standalone anti-memory CLI preserved: ${yesNo(report.standaloneAntiMemoryCliPreserved)}`,
  `Standalone anti-memory MCP preserved: ${yesNo(report.standaloneAntiMemoryMcpPreserved)}`,
  `Standalone anti-memory usefulness row delta: ${report.standaloneAntiMemoryUsefulnessRowDelta}`,
  ...
  `Selector retained control application: ${report.selectorRetainedMemoryApplicationId}`,
  ...
  `Selector packet includes retained control memory: ${yesNo(report.selectorPacketIncludesRetainedMemory)}`,
...
const runShowSmokeTarget: DbSmokeTargetHandler = async (
  ...
  return smokeResultFromCleanup(
    ...
    [
      ...
      `Terminal EvidenceContract activation inactive: ${yesNo(report.terminalActivationInactive)}`,
      `Terminal EvidenceContract history visible: ${yesNo(report.terminalContractHistoryVisible)}`,
      `Issued EvidenceContract retained after terminal transition: ${yesNo(report.terminalIssuedContractRetained)}`,
      `Issued DecisionPacket identity retained after terminal transition: ${yesNo(report.terminalPacketIdentityRetained)}`,
