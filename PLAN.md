# KRN Active Plan
Status: active compact root plan. Date: 2026-07-03. Detailed history stays in Beads, archived ledgers, and archived reports.
## Product State
```txt
...
corpus closure readback: connected project source decision gaps report 8/8 accepted SourceClaims linked and 11/11 non-adopted claims explicitly rejected; canonical worker/dashboard/unknown-first/bounded-loop/source-authority/feedback-forget queries surface linked governing claims in top 3
...
worker Memory Core write boundary: worker write declarations are validated and visible in DB smoke readback, but not runtime-enforced
heartbeat worker boundary readback: memory-staleness candidates expose validated expire_stale_memory write boundary
DB-backed heartbeat worker boundary: one seeded expired MemoryRecord emits a review-ready memory-staleness candidate with validated workerWriteBoundary and cleanup
...
worker job type ownership: invariant runtime fields stay centralized; DB active job type/status contracts derive from @krn/workers
...
pattern usefulness evidence link: persisted evidence capture downgrades retained-pattern usefulness unless evidence refs match current evidence/command proof
...
source graph relation ranking proof: invalidates/duplicates are DB-smoke visible; eval:source-graph-ranking covers supports/duplicates/invalidates plus held-out depends_on/qualifies relation-shape and incoming/outgoing direction readback
...
Current active Bead: `mise-en-palace-9rnq` corpus closure.
Latest closed Bead: `mise-en-palace-yb2n` notes-baseline decision-packet eval.
Next ready Beads after `9rnq`: `mise-en-palace-ovit` feedback/forget loop, `mise-en-palace-qqet` proof/docs cleanup, `mise-en-palace-rr9b` decision-packet benchmark, `mise-en-palace-v4e9` second-repo dry run.
...
`mise-en-palace-ww5l` added deterministic source artifact preview extraction -> activation context reuse to `eval:krn:smoke`.
