...
CLI scoped test typecheck: active smoke runs strict `tsc`; codex/reflect/observe fixtures joined the clean subset
...
### CLI Observe Test Fixture Typecheck Batch

Goal: expand the scoped strict TypeScript gate with the observe command test batch.
...
Latest report: `docs/runs/2026-07-03-cli-test-typecheck-observe-batch.md`.

Task boundary:

```txt
packages/cli/tsconfig.tests.clean.json now includes observe.test.ts.
The observe test fixture matches runtime interfaces without casts or assertion changes.
