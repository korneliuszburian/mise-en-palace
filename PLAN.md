...
```txt
mise-en-palace-5vc: Run compact multi-repo Brain-QA batch.
```
...
IMR-18-46: AMA evidence and activation utility lanes accepted; natural recall and source-backed selected knowledge repaired; source artifact preview emits structured JSON, heartbeat consumes it as acquisition input, persisted artifact/chunk/SearchDocument readback resolves the candidate, and one second-repo file now reads back through source/brain search.
```
