...
- Slice note (2026-06-23): implement a small CLI adapter boundary for
  filesystem existence, repo-root discovery, and JSON-object reads. Intended
  files: `packages/cli/src/cliFileBoundary.ts`,
  ...
  `packages/cli/src/runDbSmokeCommand.ts`, status docs, and this PLAN.
  Non-goals: no command parser modularization, no doctor check split, no DB
  schema/migration, no memory/eval feature work, no Promptfoo decision, no
  dashboard/API/MCP/server/plugin/source crawler.
- Verification:
...
- Feed QG-04's accepted scans into QG-06 audit automation: placeholder
  vocabulary, duplicate helper families, over-broad metadata / behavior-governing
  metadata keys, large-file thresholds, and accepted allowlists with expiry.
- Metadata rule to automate: `metadata` may carry audit/debug/compatibility
  payloads, but any key that affects governance, ranking, promotion, review,
  activation, source lineage, or Memory Core semantics must be promoted to an
  explicit typed field or be reported as a smell finding.
- Verification:
...
Only after QG-04B through QG-06 are complete may the plan continue to MM-66.
...
- [x] (2026-06-23) MM-61 complete: added memory behavior golden cases and fixture-backed harness behavior tests without introducing the broad golden runner. Intended files: `tests/fixtures/golden-tasks/memory-behavior.json`, `packages/harness/src/activation/goldenMemoryBehavior.test.ts`, `packages/schema/src/goldenTask.test.ts`, root `PLAN.md`, `GOAL.md`, handoff files, and this PLAN. Non-goals preserved: no runner, no CLI, no DB schema/migration, no repository, no Promptfoo export, no broad benchmark suite, no dashboard/API/MCP/server/plugin/source crawler. Evidence: RED focused harness test first failed because the fixture declared only three memory behavior cases; after adding temporal-validity and application-guidance cases, focused harness test passed with 10 files / 56 tests; focused schema fixture test passed with 2 files / 21 tests after updating deterministic task-order expectations. Tests protect: source-linked memory selection, stale memory exclusion and abstention when stale is the only option, weak memory abstention, current memory selection over expired memory, and application guidance preservation in ranked memory candidates. Next: MM-61-lite early golden memory smoke cases for stale memory abstain, anti-memory block, and unsupported SourceDecision rejection.
- [x] (2026-06-23) MM-61-lite complete: added early golden memory smoke cases for stale memory abstain, anti-memory block, and unsupported SourceDecision rejection before the broad golden runner. Intended files: `tests/fixtures/golden-tasks/memory-behavior.json`, `packages/harness/src/activation/goldenMemoryBehavior.test.ts`, `packages/schema/src/goldenTask.test.ts`, root `PLAN.md`, `GOAL.md`, handoff files, and this PLAN. Non-goals preserved: no golden runner, no CLI, no DB schema/migration, no repository, no Promptfoo export, no broad benchmark suite, no dashboard/API/MCP/server/plugin/source crawler. Evidence: RED focused harness golden behavior test failed because the fixture lacked `golden-case-memory-smoke-001`, `golden-case-memory-smoke-002`, and `golden-case-source-smoke-001`; GREEN focused harness test passed with 10 files / 59 tests after adding behavior smoke tests for stale abstention, explicit anti-memory blocking, and source grounding audit rejection of unsupported SourceDecision; focused schema fixture test passed with 2 files / 21 tests after deterministic task/case expectations included the source smoke fixture. Next: MM-62 context/source/audit/type boundary golden cases.
...
- [x] (2026-06-23) QG-00 complete: added a repo-wide current-state inventory and synchronized current-state docs so fresh reviewers do not build from stale assumptions. Intended files: `docs/plans/memory-ideal-state/QG-00-REPO-INVENTORY.md`, `README.md`, `GOAL.md`, root `PLAN.md`, `REVIEW.md`, `docs/handoff/*`, and this PLAN. Non-goals preserved: no TypeScript source change, no schema/migration, no CLI/runtime behavior, no DB write, no worker, no dashboard/API/MCP/server/plugin/source crawler, no broad eval suite, no memory promotion, no runtime markdown memory, no Research Foundry, no Pattern Vault. Evidence: preflight `git status --short --branch` showed branch aligned with `origin/main` plus only the raw research materials untracked; `git log --oneline -8` showed latest `7e0c6b8`; `git rev-parse HEAD` returned `7e0c6b8691c79e4ccba85bae9ac004f2fe63b5ad`; `pnpm typecheck` passed; `pnpm test` passed with 56 test files; inventory commands counted 162 production TS files and 56 colocated test files across packages; forbidden directory scan found no `packages/research-foundry`, `packages/pattern-vault`, `apps/dashboard`, `packages/api`, `packages/mcp`, or `.krn`; QG-00 recorded current package/test/export/script/docs/non-goal facts, identified unresolved engineering inconsistencies, and marked `REVIEW.md` as historical. Next: QG-01 test topology ADR and enforcement.
...
- [x] (2026-06-23) QG-04A complete: consolidated duplicated CLI filesystem and JSON boundary helpers into `packages/cli/src/cliFileBoundary.ts`. Intended files: `packages/cli/src/cliFileBoundary.ts`, `packages/cli/src/cliFileBoundary.test.ts`, `packages/cli/src/runDoctorCommand.ts`, `packages/cli/src/runInitCommand.ts`, `packages/cli/src/runDbReadinessCommand.ts`, `packages/cli/src/runDbSmokeCommand.ts`, status docs, and this PLAN. Non-goals preserved: no command parser modularization, no doctor check split, no DB schema/migration, no memory/eval feature work, no Promptfoo decision, no dashboard/API/MCP/server/plugin/source crawler. Evidence: RED focused `pnpm --filter @krn/cli test -- cliFileBoundary` failed because `./cliFileBoundary.js` did not exist; GREEN focused test passed with 7 files / 96 tests after adding `pathExists`, `findRepoRoot`, and unknown-first `readJsonObject`; duplicate-helper scan found only the canonical helper definitions in `cliFileBoundary.ts`; focused `pnpm --filter @krn/cli test` passed with 7 files / 96 tests; focused `pnpm --filter @krn/cli typecheck` passed; full `pnpm typecheck` passed; full `pnpm test` passed; `git diff --check` passed; forbidden directory scan found no added forbidden surfaces; stale status scan passed; `krn audit slice --since origin/main --fail-on warning` passed with verdict `pass` and 0 findings. Next: QG-04B command parser modularization.
- [x] (2026-06-23) QG-04B complete: modularized CLI command-family parsing so `packages/cli/src/parseArgs.ts` is now a dispatcher plus shared `CliCommand`/top-level usage surface rather than the owner of every command grammar. Intended files: `packages/cli/src/parseArgs.ts`, extracted parser modules/tests, `packages/cli/src/runCli.ts`, status docs, and this PLAN. Non-goals preserved: no command semantics change, no memory confidence parser consolidation, no doctor runtime split, no DB schema/migration, no memory/eval feature work, no Promptfoo decision, no dashboard/API/MCP/server/plugin/source crawler. Evidence: parser families extracted for audit, db, init, evidence, observe, codex, reflect, review, plan, doctor, source, and memory; final memory RED focused `pnpm --filter @krn/cli test -- parseMemoryArgs` failed because `./parseMemoryArgs.js` did not exist; GREEN focused memory parser test passed with 19 files / 129 tests; focused CLI typecheck passed; focused CLI test passed with 19 files / 129 tests; full `pnpm typecheck` passed; full `pnpm test` passed; `git diff --check` passed; forbidden directory scan found no added forbidden surfaces; `krn audit slice --since origin/main --fail-on warning` passed with verdict `pass` and 0 findings; `parseArgs.ts` was reduced from 2181 lines at QG-04 audit time to 379 lines and no longer imports `metadataEntry` or `optionValue`. Next: QG-04C doctor command modularization.
- [x] (2026-06-23) QG-04C complete: modularized `krn doctor` so `packages/cli/src/runDoctorCommand.ts` is now a 205-line orchestration/rendering entrypoint. Intended files: `packages/cli/src/runDoctorCommand.ts`, `packages/cli/src/doctorRepoChecks.ts`, `packages/cli/src/doctorStaticChecks.ts`, `packages/cli/src/doctorDbChecks.ts`, `packages/cli/src/doctorReadiness.ts`, `packages/cli/src/doctorCheckHelpers.ts`, focused tests, status docs, and this PLAN. Non-goals preserved: no doctor behavior change, no DB schema/migration, no memory/eval feature work, no Promptfoo decision, no dashboard/API/MCP/server/plugin/source crawler. Evidence: RED focused `pnpm --filter @krn/cli test -- doctorRepoChecks` failed because `./doctorRepoChecks.js` did not exist; GREEN focused repo-check test passed with 20 files / 131 tests; RED focused `pnpm --filter @krn/cli test -- doctorStaticChecks` failed because `./doctorStaticChecks.js` did not exist; GREEN focused static-check test passed with 21 files / 133 tests; RED focused `pnpm --filter @krn/cli test -- doctorDbChecks` failed because `./doctorDbChecks.js` did not exist; GREEN focused DB-check test passed with 22 files / 135 tests; RED focused `pnpm --filter @krn/cli test -- doctorReadiness` failed because `./doctorReadiness.js` did not exist; GREEN focused readiness test passed with 23 files / 137 tests; focused CLI typecheck passed; focused CLI test passed with 23 files / 137 tests; read-only module tests block write imports and shell execution paths. Next: QG-04D memory confidence parser consolidation.
- [x] (2026-06-23) QG-04D complete: consolidated MemoryCandidate and AntiMemory CLI confidence parsing into `packages/cli/src/parseMemoryConfidence.ts`. Intended files: `packages/cli/src/parseMemoryConfidence.ts`, `packages/cli/src/parseMemoryConfidence.test.ts`, `packages/cli/src/runMemoryCandidateAddCommand.ts`, `packages/cli/src/runMemoryAntiAddCommand.ts`, status docs, and this PLAN. Non-goals preserved: no CLI grammar change, no schema/migration, no memory promotion, no confidence model redesign, no Promptfoo decision, no dashboard/API/MCP/server/plugin/source crawler. Evidence: RED focused `pnpm --filter @krn/cli test -- parseMemoryConfidence` failed because `./parseMemoryConfidence.js` did not exist; GREEN focused parser test passed with 24 files / 140 tests after adding the shared parser; focused CLI typecheck passed; focused `pnpm --filter @krn/cli test -- runCli` passed with 24 files / 140 tests; focused `pnpm --filter @krn/cli test -- parseMemoryArgs` passed with 24 files / 140 tests; duplicate scan shows both memory write commands import `parseMemoryConfidence` and no longer define local `confidenceAliases` or `parseConfidence`. Next: QG-04E schema metadata guard consolidation.
- [x] (2026-06-23) QG-04E complete: consolidated schema-local metadata/text/list primitives and private reasoning metadata guards into `packages/schema/src/schemaPrimitives.ts`. Intended files: `packages/schema/src/schemaPrimitives.ts`, `packages/schema/src/schemaPrimitives.test.ts`, schema modules using metadata/text/list primitives, status docs, and this PLAN. Non-goals preserved: no core runtime import into schema, no metadata semantics redesign, no schema/DB migration, no memory/eval feature work, no Promptfoo decision, no dashboard/API/MCP/server/plugin/source crawler. Evidence: RED focused `pnpm --filter @krn/schema test -- schemaPrimitives` failed because `./schemaPrimitives.js` did not exist; GREEN focused schema primitive test passed with 3 files / 25 tests; focused schema typecheck passed; focused schema test passed with 3 files / 25 tests; duplicate primitive scan shows only `schemaPrimitives.ts` defines `MetadataSchema`, `RequiredTextSchema`, `OptionalTextSchema`, and `TextListSchema`; domain-specific forbidden metadata keys remain local only where needed, such as reflection final-truth mutation keys. Next: QG-04F review signal vocabulary consolidation.
- [x] (2026-06-23) QG-04F complete: consolidated repeated review status/outcome/risk vocabulary and review metadata readers into `packages/core/src/reviewSignal.ts`. Intended files: `packages/core/src/reviewSignal.ts`, `packages/core/src/reviewSignal.test.ts`, `packages/core/src/reviewAssessment.ts`, `packages/core/src/feedbackDelta.ts`, `packages/core/src/index.ts`, `packages/cli/src/runReviewAssessCommand.ts`, status docs, and this PLAN. Non-goals preserved: no repository change, no schema/DB migration, no review semantics redesign, no memory promotion, no Promptfoo decision, no dashboard/API/MCP/server/plugin/source crawler. Evidence: RED focused `pnpm --filter @krn/core test -- reviewSignal` failed because `./reviewSignal.js` did not exist; GREEN focused `pnpm --filter @krn/core test -- reviewSignal reviewFeedback` passed with 8 files / 39 tests; focused `pnpm --filter @krn/cli test -- runCli parseReviewArgs` passed with 24 files / 140 tests; focused core and CLI typechecks passed; duplicate review-vocabulary scan shows ReviewAssessment, FeedbackDelta, and `runReviewAssessCommand.ts` no longer define local review outcome/risk sets or local outcome/risk normalizers. Next: QG-04G DB mapper/repository split plan and first safe split.
- [x] (2026-06-23) QG-04G complete: split memory-domain row mapping out of `packages/db/src/repositories/mappers.ts` into `packages/db/src/repositories/memoryMappers.ts`, kept the legacy mapper import surface through re-exports, and added focused memory mapper tests. Intended files: `packages/db/src/repositories/memoryMappers.ts`, `packages/db/src/repositories/memoryMappers.test.ts`, `packages/db/src/repositories/mappers.ts`, status docs, and this PLAN. Non-goals preserved: no DB schema/migration, no repository interface change, no Memory Core behavior change, no mapper import churn beyond the compatibility re-export, no Promptfoo decision, no dashboard/API/MCP/server/plugin/source crawler. Evidence: RED focused `pnpm --filter @krn/db test -- memoryMappers` failed before `memoryMappers.ts` existed; pre-completion `pnpm test` then failed only on the new incomplete row fixture; GREEN focused `pnpm --filter @krn/db test -- memoryMappers` passed with 24 files / 67 tests; focused `pnpm --filter @krn/db test -- mappers` passed with 24 files / 67 tests; focused DB typecheck passed; final `pnpm typecheck`, `pnpm test`, `git diff --check`, forbidden directory scan, and `krn audit slice --since origin/main --fail-on warning` passed with verdict `pass` and 0 findings; `mappers.ts` dropped from 939 lines at QG-04 audit time to 714 lines, while `memoryMappers.ts` owns the memory row types, source-lineage narrowing, memory candidate JSON narrowing, and memory/anti-memory mapping. Next: QG-04H smell scan automation requirements.
- [x] (2026-06-23) QG-04H complete: recorded concrete smell-scan automation requirements for QG-06 at `docs/plans/memory-ideal-state/QG-04H-SMELL-SCAN-AUTOMATION-REQUIREMENTS.md`. Intended files: the QG-04H requirements doc, root/current status docs, and this PLAN. Non-goals preserved: no audit runtime implementation, no new dependency, no Promptfoo decision, no package topology change, no DB schema/migration, no Memory Core mutation, no dashboard/API/MCP/server/plugin/source crawler. Evidence: preflight `git status --short --branch` showed branch aligned with `origin/main` plus only raw research materials untracked; preflight `pnpm typecheck` passed; preflight `pnpm test` passed; QG-04H requirements define QG-06 audit categories for placeholder vocabulary, duplicate helper families, large-file thresholds, behavior-governing metadata keys, export/dead-code scan, stale current-state docs, allowlist shape, and seeded failure proofs; final `pnpm typecheck`, `pnpm test`, `git diff --check`, forbidden directory scan, and `krn audit slice --since origin/main --fail-on warning` passed with verdict `pass` and 0 findings. Next: QG-05 official Promptfoo integration decision.
- [x] (2026-06-23) QG-05 complete: adopted official Promptfoo as a bounded eval-lane runner and result adapter at `docs/plans/memory-ideal-state/QG-05-PROMPTFOO-DECISION.md`. Intended files: QG-05 decision doc, official Promptfoo smoke fixture/provider, root `package.json`/lockfile, harness Promptfoo-result bridge, status docs, and this PLAN. Non-goals preserved: Promptfoo is not Memory Core, not GoldenTask truth, not EvalCandidate truth, not a review gate, not MCP/server/share/redteam integration, and not a broad benchmark suite. Evidence: official source-to-decision mapping recorded; `promptfoo@0.121.17` installed as a dev dependency; RED focused `pnpm --filter @krn/harness test -- goldenPromptfooResult.test.ts` failed before `goldenPromptfooResult.ts` existed; GREEN focused harness test passed with 15 files / 81 tests; `pnpm exec promptfoo --version` returned `0.121.17`; `pnpm exec promptfoo validate config -c tests/fixtures/promptfoo/krn-golden-smoke.yaml` passed with "Configuration is valid"; `pnpm eval:promptfoo:smoke` passed 2/2 cases and wrote `.local-lab/promptfoo/krn-golden-smoke-results.jsonl`; full `pnpm typecheck`, full `pnpm test`, `git diff --check`, and forbidden directory scan passed; `krn audit slice --since origin/main --fail-on warning` is expectedly blocked by the known pre-QG-06 missing intended-files/verification-commands warnings, while the same audit without fail-on-warning exits 0 with verdict `advisory`, 0 blocking findings, and those 2 warnings only; Promptfoo emitted non-blocking `DecompressInterceptor` and telemetry-shutdown warnings documented in the decision. Next: QG-06 quality gate automation in `krn audit`.
- [ ] QG-06: quality gate automation in `krn audit`.
