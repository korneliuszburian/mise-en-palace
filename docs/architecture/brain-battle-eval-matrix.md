...
| KRN lifecycle/state-dependent domain models should use narrow unions or discriminated unions when valid fields differ by state. | candidate | Run a bounded TypeScript lifecycle union drift spot-check before promoting a deterministic guard. | `docs/KRN_SOURCES.md#unions-literals-and-narrowing`; `docs/runbooks/pattern-intake.md`; `docs/reviews/controlled-dogfood/2026-06-27-v59-first-pattern-intake-runbook-application/REPORT.md`. | Does not prove current TypeScript drift, that every object needs a discriminant, or that a broad type rewrite is valuable. |
