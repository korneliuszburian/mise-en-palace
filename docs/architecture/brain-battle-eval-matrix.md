...
| SourceClaim and SourceDecision records require mechanism, KRN implication, consumer, falsifier or `doesNotProve`, and reject decorative source retention. | candidate only | Existing schema/source CLI tests cover parts; a future guard should promote a behavior case after another dogfood shows repeated risk. | `packages/schema/src/source.ts`; source CLI tests; `docs/architecture/security-trust-boundaries.md`. | Current matrix does not prove source selection quality. |
...
| Run readback distinguishes proof from non-proof and can be used by operator or Promptfoo without ad hoc SQL. | candidate only | Existing CLI readback tests cover JSON/resource output; future eval should promote a run-readback behavior fixture if it repeatedly blocks review. | `packages/cli/src/runRunShowCommand.ts`; `packages/cli/src/runRunShowCommand.test.ts`; `docs/reviews/controlled-dogfood/2026-06-25-run-evidence-readback/REPORT.md`. | Does not prove live DB readback unless DB commands run. |
...
trust exclusion case. It is deterministic and does not call an LLM judge.
