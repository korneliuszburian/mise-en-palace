...
| Embedding model provenance in vector retrieval | closed by evidence | Current repository inputs require `embeddingModelId`, runtime guards reject missing ids, SQL scopes vector rows by model, and vector/hybrid readback exposes model provenance. | Reopen only if a caller can invoke vector/hybrid retrieval without a model id or compare embeddings across models silently. |
...
- `mise-en-palace-lgqi`: retrieval: embedding model provenance for vector search closed by code/test evidence
...
| `mise-en-palace-lgqi` | P1 | none | Done by evidence: vector/hybrid retrieval requires `embeddingModelId`; readback exposes provenance; missing model id is tested. | `pnpm --filter @krn/db test -- DrizzleRetrievalRepository`; `pnpm -C packages/db typecheck`; CI DB retrieval smoke. | New vector provider, embedding generation service, broad retrieval rewrite. |
