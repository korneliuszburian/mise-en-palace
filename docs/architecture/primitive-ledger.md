...
CI-invoked guard in `KRN_ROADMAP.md`; a `rejected`,
...
| --- | --- | --- | --- | --- |
| select: activation retrieval, ranking, filtering | live | `KRN_ROADMAP.md`; `packages/harness/src/activation/activationEngine.ts`; `packages/harness/src/activation/rankCandidates.ts`; `packages/harness/src/activation/activationFilters.ts` | `pnpm eval:krn:smoke`; `pnpm db:smoke:brain-loop`; `pnpm db:smoke:source-graph` | Does not prove optimal ranking or arbitrary target coverage. |
| apply: compile plan, assemble context, record memory application | live | `KRN_ROADMAP.md`; `packages/harness/src/compiler/compileHarnessPlan.ts`; `packages/harness/src/activation/assembleContext.ts`; `packages/db/src/repositories/DrizzleMemoryRepository.ts` | `pnpm eval:krn:smoke`; `pnpm db:smoke:brain-loop` | Does not prove Codex used the context well. |
| verify: evidence, review, feedback, context, activation readback | live | `KRN_ROADMAP.md`; `packages/db/src/brainLoopSmoke.ts`; `packages/db/src/repositories/DrizzleHarnessRunRepository.ts` | `pnpm eval:krn:smoke`; `pnpm db:smoke:brain-loop` | Does not prove review judgment or product readiness. |
| forget: hurt/stale feedback and anti-memory exclusion | live | `KRN_ROADMAP.md`; `packages/core/src/memory.ts`; `packages/harness/src/activation/activationFilters.ts`; `packages/db/src/brainLoopSmoke.ts` | `pnpm eval:krn:smoke`; `pnpm db:smoke:brain-loop` | Does not prove autonomous pruning or complete stale-claim detection. |
...
| Company-pattern memory advantage eval | live, proxy | `pnpm eval:memory-advantage`; `pnpm eval:codex-output-comparator`; `tests/fixtures/memory-advantage/company-pattern-memory-advantage.json` | Controlled no-memory/simple-lexical/plan-brief baselines plus Codex-vs-KRN execution-contract comparator; not arbitrary task superiority, ranking quality, live Codex behavior, or LLM output quality. |
| Notes-baseline decision-packet eval | live, proxy | `pnpm eval:notes-baseline`; `tests/fixtures/notes-baseline/decision-packet-vs-notes.json` | Compares KRN packets against comprehensive notes+grep on self-repo task framings; not live Codex obedience, paid-product proof, source truth, or arbitrary-repo advantage. |
| DB-backed memory advantage smoke | live, narrow | `pnpm db:smoke:brain-search`; `packages/cli/src/runBrainSearchDbSmoke.ts` | One isolated Postgres project replays `firm-pattern-store-backed-memory-no-markdown`: baseline miss, Session A evidence/review/feedback, MemoryCandidate promotion, MemoryRecord readback, SourceClaim, SearchDocument, SourceDecisionEdge support, source contribution, limitation classification, and cleanup; not broad memory ranking quality. |
...
| --- | --- | --- | --- |
| Broad dashboard/API/MCP/product surface | rejected | `KRN_ROADMAP.md`; root active plans | Do not build before live kernel primitives need it. |
...
| File-backed runtime markdown memory | rejected | `KRN_ROADMAP.md` runtime truth | Markdown may be source/export/audit/seed/backup, not runtime project memory. |
...
| Historical docs/materials as active context | deprecated | `KRN_ROADMAP.md`; context hygiene invariants | Read only task-relevant active docs; do not broad-reread ledgers by default. |
