...
| Company-pattern memory advantage eval | live, proxy | `pnpm eval:memory-advantage`; `tests/fixtures/memory-advantage/company-pattern-memory-advantage.json` | Controlled no-memory and simple-lexical baselines vs KRN memory/source hit with distractor readback; not arbitrary task superiority or LLM output quality. |
