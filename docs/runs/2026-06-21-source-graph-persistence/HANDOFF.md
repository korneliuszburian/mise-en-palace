...
Last verified state:
M22 Slice 05 source graph smoke is complete. `GOAL.md` defines M22. Slice 00
...
- `packages/db/src/sourceGraphSmoke.ts`
...
`pnpm db:smoke:source-graph` the live persistence proof.

Blockers/risks:
No Slice 05 blocker. CLI source commands, evidence source candidates, and
