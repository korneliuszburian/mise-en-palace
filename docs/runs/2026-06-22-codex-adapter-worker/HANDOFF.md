...
Skeleton. M26.00 inventory is complete; next implementation slice is M26.01
...
`git diff --check` passed after adding the M26.00 ledger.
...
`CodexAdapterPlanRef`. Use existing Postgres `worker_jobs` and `outbox_events`
for worker skeleton proof; do not add Redis/Kafka or a daemon. Treat
`embed_memory_record`, `skipped`, and `availableAt` versus `runAfter` as
...
Next action:
Commit `docs(run): record Codex adapter and worker inventory`, push, then start
M26.01 Codex adapter contracts.
