...
```txt
CLI doctor test typecheck widening slice is active.
The scoped CLI test typecheck gate now includes doctor readiness/check tests.
Doctor, smoke/readiness, helper, parser, and targeted command-group fixture batches are now in the clean subset.
```

Latest outcome:

```txt
CLI doctor test typecheck widening slice: `doctorDbChecks`, `doctorReadiness`,
`doctorRepoChecks`, and `doctorStaticChecks` now satisfy the scoped strict
...
- [x] EVD-02 complete: schema evidence command parsing now delegates command
  normalization semantics to `@krn/core`, reducing duplicate proof/non-proof
  logic between packages. Report:
...
- [x] EVD-03 complete: the canonical core evidence command normalizer now uses
  ...
  Next: `mise-en-palace-yem` / ACT-01 wires blocking memory review signals into
  activation exclusion.
- [x] ACT-01 complete: blocking memory review signals now structurally exclude
  memory candidates from activation context while warning-only signals remain
  reviewable metadata. Report:
