...
## Outcome 2026-06-27 V179 Infra ADR Skill Metadata Guard

Completed tasks:
- V178-00 Post Pattern Chain Smoke Re-Gate.
- V179-00 Infra ADR Skill Metadata Guard.

V178 decision:
- Do not broaden smoke coverage into Research Foundry, source crawler,
  dashboard, or product-readiness claims.
- Select target infra ADR skill metadata repair because Codex sees frontmatter
  routing metadata before loading the full skill body, and the description still
  named adoption/rejection/falsifier without consumer.
- The concrete failure mode is a future infra decision selecting the skill from
  metadata that does not surface consumer discipline.

What changed:
- `.agents/skills/target-infra-adr/SKILL.md` frontmatter description now names
  explicit adoption, rejection, consumer, and falsifier.
- `packages/harness/src/skillInvariants.test.ts` now guards that routing
  metadata.
- `docs/architecture/brain-battle-eval-matrix.md` records that the target infra
  ADR skill guard includes frontmatter routing metadata.
- Root `GOAL.md`, root `PLAN.md`, and `PLANS.md` now point at V180.

Evidence:
- Before this slice, the skill body and output template carried `consumer`, but
  the frontmatter description only named adoption, rejection, and falsifier.
- Frontmatter descriptions are the progressive-disclosure routing surface.

Source-to-decision:
- Source: `.agents/skills/target-infra-adr/SKILL.md` and repo-local skill
  invariant coverage.
- Mechanism: skill descriptions determine when Codex loads the skill; if routing
  metadata omits consumer, the first visible contract is weaker than the skill
  body and pattern gate.
- KRN implication: high-risk infra/runtimes skill metadata must include
  consumer before falsifier, not only the loaded workflow.
- Decision: update and guard target infra ADR skill frontmatter.
- Does not prove: every future infra decision invokes the skill, product
  readiness, or that a new runtime surface should be adopted.
- Consumer: `.agents/skills/target-infra-adr/SKILL.md`,
  `packages/harness/src/skillInvariants.test.ts`, and
  `docs/architecture/brain-battle-eval-matrix.md`.
- Falsifier: the target infra ADR frontmatter can drop consumer while tests pass.
...
## Outcome 2026-06-27 V195 Onboarding Current-State Repair

Completed task:
- V195-00 Onboarding Current-State Repair.

What changed:
- `docs/KRN_ONBOARDING.md` now states the current controlled-internal-alpha
  status instead of Commit 0/1 bootstrap state.
- The onboarding first-read path points to active `GOAL.md`, `PLAN.md`, and
  `PLANS.md`, then a bounded repair/guard, evidence, next-task update, commit,
  push, and CI check when relevant.
- The onboarding doc keeps the source/pattern chain:
  source -> mechanism -> KRN implication -> decision/rejection -> consumer ->
  falsifier.
- `packages/harness/src/contextHygieneInvariants.test.ts` now fails if
  onboarding returns to Commit 0/1-only language or stale `krn context build` /
  `krn review capture` commands.

Evidence:
- Focused context hygiene test passed with TMPDIR outside the repo.
- The test preserves current-state onboarding as a regression-visible operator
  surface.

Source-to-decision:
- Source: `docs/KRN_ONBOARDING.md`, root `GOAL.md` / `PLAN.md`, and context
  hygiene invariants.
- Mechanism: onboarding is an operator-facing context selector; stale bootstrap
  wording can make new continuations misunderstand KRN's stage even when root
  active truth is correct.
- KRN implication: current-state condensation must include small onboarding
  surfaces that fresh operators actually read.
- Decision: repair onboarding and guard it against stale bootstrap command
  regression.
- Does not prove: every operator-facing doc is current, product readiness, or
  widened internal alpha.
- Consumer: `docs/KRN_ONBOARDING.md` and
  `packages/harness/src/contextHygieneInvariants.test.ts`.
- Falsifier: onboarding can again claim only Commit 0/1 surfaces or show stale
  first-dogfood commands while tests pass.

New task:
- V196-00 Post Onboarding Current-State Re-Gate.
...
## Outcome 2026-06-27 V196 State Of The Art Re-Gate

Completed task:
- V196-00 Post Onboarding Current-State Re-Gate.

Decision:
- Do not broaden onboarding repair into a docs sweep, Research Foundry, source
  crawler, dashboard, product-readiness claim, or architecture rewrite.
- Select a focused state-of-the-art doctrine repair because
  `docs/STATE_OF_THE_ART.md` is still labeled active doctrine while its
  replacement path uses older `context packet` / `review capture` language.

Evidence:
- `docs/KRN_ONBOARDING.md` is now current and guarded.
- `docs/STATE_OF_THE_ART.md` still presents an older linear path that predates
  the current canonical harness spine and continuous evidence loop.

Source-to-decision:
- Source: `docs/STATE_OF_THE_ART.md`, `docs/KRN_KERNEL.md`, root
  `GOAL.md` / `PLAN.md`, and V195 onboarding repair.
- Mechanism: active doctrine shapes operator mental models; stale path language
  can make current KRN look earlier than it is or route work through old command
  names.
- KRN implication: state-of-the-art doctrine should point at the current
  controlled-internal-alpha spine and pattern gate while preserving product
  non-readiness.
- Decision: repair `docs/STATE_OF_THE_ART.md` current-state path and guard it if
  a small invariant is justified.
- Does not prove: every docs page is current, product readiness, or need for a
  broad docs sweep.
- Consumer: `docs/STATE_OF_THE_ART.md` and a focused invariant if needed.
- Falsifier: state-of-the-art doctrine can again route operators through stale
  `context packet` / `review capture` language while tests pass.
...
## Outcome 2026-06-28 V200 README Current-State Re-Gate

Completed task:
- V200-00 Post Kernel Boundary Re-Gate.

Decision:
- Do not broaden kernel repair into a docs sweep, Research Foundry, source
  crawler, dashboard, product-readiness claim, or architecture rewrite.
- Select README current-state repair because `README.md` is operator-facing and
  still frames the repo around reset/QG/audit cleanup rather than current
  controlled-internal-alpha continuous brain growth.

Evidence:
- `docs/KRN_ONBOARDING.md`, `docs/STATE_OF_THE_ART.md`, and
  `docs/KRN_KERNEL.md` are now current and guarded.
- `README.md` still says "The current reset direction is" and centers QG /
  anti-slop / audit-authority cleanup as current direction.

Source-to-decision:
- Source: `README.md`, root `PLAN.md`, V195 onboarding repair, V197 state
  doctrine repair, and V199 kernel repair.
- Mechanism: README is the broadest operator-facing entrypoint; stale reset
  framing can make future work chase already-closed cleanup instead of current
  continuous evidence-driven hardening.
- KRN implication: README should point to compact active truth, current
  controlled-internal-alpha status, and continuous pattern gate without
  resurrecting reset/audit as current work.
- Decision: repair README current-state framing and guard it.
- Does not prove: product readiness, every docs page is current, or README is a
  full execution plan.
- Consumer: `README.md` and focused context hygiene invariant.
...
## Outcome 2026-06-28 V203 TypeScript Boundary Current-State Repair

Completed task:
- V203-00 TypeScript Boundary Current-State Repair.

What changed:
- `docs/standards/typescript-boundaries.md` no longer says TypeScript code
  starts only after Commit 0/1.
- The standard now states it applies to every current implementation slice that
  touches TypeScript source, public types, validators, config, CLI input,
  app/connector input, MCP/tool input, plugin output, or generated data.
- The standard now frames the goal as continuous boundary discipline, not broad
  type cleanup.
- `packages/harness/src/typescriptBoundaryInvariants.test.ts` now guards the
  standard against returning to Commit 0/1 stage language.

Evidence:
- Focused TypeScript boundary invariant test passed with TMPDIR outside the
  repo.
- The existing production TypeScript boundary scans remain in the same test
  file.
...
- Source: `docs/standards/typescript-boundaries.md`,
  `.agents/skills/typescript-type-safety/SKILL.md`, and TypeScript boundary
  invariants.
- Mechanism: TypeScript implementation slices use this standard before source
  changes; stale stage language can make current TS work look out-of-phase.
- KRN implication: TypeScript boundary rules are current continuous discipline,
  not bootstrap-only sequencing.
- Decision: repair the standard and guard it.
- Does not prove: every TypeScript model is ideal, product readiness, or need
  for broad type rewrites.
- Consumer: `docs/standards/typescript-boundaries.md` and
  `packages/harness/src/typescriptBoundaryInvariants.test.ts`.
- Falsifier: the standard can again describe Commit 0/1 as current coding stage
  while tests pass.

New task:
- V204-00 Post TypeScript Boundary Re-Gate.
...
## Outcome 2026-06-28 V204 Source Map Iterative Repair Re-Gate

Completed task:
- V204-00 Post TypeScript Boundary Re-Gate.

Decision:
- Do not broaden TypeScript standard repair into a broad type rewrite, Research
  Foundry, source crawler, dashboard, product-readiness claim, or docs sweep.
- Select source-map iterative repair current-state repair because
  `docs/KRN_SOURCES.md` is retained source guidance and still says iterative
  repair means "review capture" and later dogfood starts with `krn doctor`.

Evidence:
- The active implementation loop now uses evidence/review/feedback commands,
  bounded active tasks, and continuous PLANS outcomes.
- `docs/KRN_SOURCES.md` is guarded for source-to-decision fields but not yet
  guarded against this stale current-state wording.

Source-to-decision:
- Source: `docs/KRN_SOURCES.md#iterative-repair-loops`,
  root `PLAN.md`, and source-map invariants.
- Mechanism: retained sources shape future decisions; stale source-map wording
  can route future repair-loop tasks through old command names or a doctor-first
  path.
- KRN implication: the source map should preserve the mechanism from the
  official pattern while mapping it to current KRN evidence/review/feedback
  surfaces.
- Decision: repair the source-map entry and guard it.
- Does not prove: every source-map decision is current, product readiness, or
  need for broad source-map rewrite.
- Consumer: `docs/KRN_SOURCES.md` and `packages/harness/src/sourceMapInvariants.test.ts`.
- Falsifier: the retained source map can again route iterative repair through
  stale review-capture or doctor-first wording while tests pass.
...
## Outcome 2026-06-28 V220 Post Infra Pattern Intake Re-Gate

Completed task:
- V220-00 Post Infra Pattern Intake Re-Gate.

Decision:
- ADR alignment was needed because `docs/KRN_SOURCES.md` named ADR-0015 as the
  consumer for the PostgreSQL row-locking source, but ADR-0015 did not yet carry
  that retained mechanism.
- Add a focused ADR invariant so the worker runtime boundary keeps the
  row-locking mechanism, consumer, and non-proof boundary visible.
- Worker runtime remains deferred.

Evidence:
- `docs/KRN_SOURCES.md#postgresql-row-locking-for-queue-like-tables` retains
  PostgreSQL row locking as official-docs infra source evidence.
- `docs/decisions/ADR-0015-worker-runtime-boundary.md` now includes
  `source_id: postgres-row-locking-skip-locked`, consumer, non-proof boundary,
  and `SKIP LOCKED` rejection language.
- `packages/harness/src/adrInvariants.test.ts` now checks ADR-0015 retains the
  row-locking source and does not treat it as permission to build a daemon.

Source-to-decision:
- Source: PostgreSQL row-locking source map entry, ADR-0015, and ADR invariant.
- Mechanism: source-map retention names a consumer, but the consumer document
  must carry the mechanism if future worker-runtime work will use it safely.
- KRN implication: infra pattern intake should land in the architecture boundary
  that will govern future implementation, not only in the source map.
- Decision: align ADR-0015 and add a focused invariant; do not build worker
  runtime.
- Does not prove: worker readiness, queue throughput, product readiness, or that
  `SKIP LOCKED` is correct for every KRN read path.
- Consumer: ADR-0015 and `packages/harness/src/adrInvariants.test.ts`.
- Falsifier: ADR-0015 drops the row-locking mechanism/non-proof boundary or a
  future worker slice treats `SKIP LOCKED` as sufficient runtime proof.

New task:
- V221-00 Post Worker Boundary ADR Alignment Re-Gate.
...
## Outcome 2026-06-28 V221 Post Worker Boundary ADR Alignment Re-Gate

Completed task:
- V221-00 Post Worker Boundary ADR Alignment Re-Gate.

Decision:
- Do not treat ADR-0015 alignment as worker readiness.
- Do not start worker runtime, queue service, daemon, dashboard, API, or MCP
  work.
- Select security/permissions/trust-boundary pattern intake as the next bounded
  surface because future MCP, hooks, workers, connectors, and target writes all
  depend on explicit trust boundaries before expansion.

Evidence:
- `docs/decisions/ADR-0015-worker-runtime-boundary.md` now carries the retained
  PostgreSQL row-locking mechanism and still defers worker runtime.
- `packages/harness/src/adrInvariants.test.ts` guards the row-locking source and
  non-proof boundary.
- GitHub Actions run `28304806015` passed for commit `f21da8b`.
- Root `PLAN.md` still rejects worker daemon, MCP, API, and dashboard surfaces
  unless a bounded evidence-backed task authorizes them.

Source-to-decision:
- Source: ADR-0015 alignment, ADR invariant, CI run `28304806015`, and root
  non-goals.
- Mechanism: infra source intake is now tied to the worker boundary; the next
  highest-risk surface is permission/trust guidance for future external
  execution or tool surfaces.
- KRN implication: security pattern intake should happen before any expansion
  into MCP, hooks, worker runtime, connectors, or broader target writes.
- Decision: open V222 Security Pattern Intake Candidate Gate.
- Does not prove: product readiness, worker readiness, MCP readiness, or that
  security guidance is complete.
- Consumer: root `PLAN.md`, root `GOAL.md`, and `PLANS.md` next-task queue.
- Falsifier: future work starts new external/runtime surfaces without a
  source-to-decision trust boundary and local falsifier.
