...
| 22 | source-to-decision | decision | `.agents/skills/source-to-decision/references/knowledge-gate.md` |
| 23 | source-to-decision | decision | `.agents/skills/source-to-decision/references/source-usefulness.md` |
| 24 | source-to-decision | decision | `.agents/skills/source-to-decision/SKILL.md` |
| 25 | target-repo-testing | checker | `.agents/skills/target-repo-testing/agents/openai.yaml` |
| 26 | target-repo-testing | checker | `.agents/skills/target-repo-testing/references/owner-file-read-model.md` |
| 27 | target-repo-testing | checker | `.agents/skills/target-repo-testing/SKILL.md` |
...
State the user/operator problem in one paragraph; no implementation plan here.

## Consumer

Who will use this spec or resulting behavior, and at what boundary?

## Solution

State the smallest settled solution that can be sliced into tickets.

## Operator/User Stories

List only stories that change acceptance criteria.

## Implementation Decisions

Name decisions already made; do not hide open questions here.

## Testing Decisions

Name the proof level expected for the ticket slice.

## Falsifiers

What would prove this spec is wrong, incomplete, or not ready to slice?

## Proof / Non-Proof

Proof:

Commands, checks, or evidence expected after implementation.

Non-proof:

Evidence that is useful but insufficient.

## Out Of Scope

Explicit work this spec must not create tickets for.

## Open Questions Blocking Slicing

Questions that must be answered before `to-tickets`; leave empty only if none.
```
...
One vertical tracer-bullet behavior, not a layer or ceremony.

## Consumer

Runtime path, operator workflow, or artifact that consumes this work.

## Acceptance Criteria

Observable conditions that make the issue closable.

## Falsifier

The test, smoke, review finding, or readback that would prove this failed.

## Proof / Non-Proof

Proof:

Commands or evidence required before close.

Non-proof:

Checks that do not prove the acceptance criteria.

## Native Dependency Edges

Required `bd dep` edges:

List blocker issue IDs that must be native Beads deps, not prose-only.

## Rollback / Contraction

How to undo, contract, or safely abandon the slice if verification fails.

## Open Questions

Questions that block implementation; create/edge a question issue when needed.
```
...
Decision, spec, or runtime change this map is trying to make possible.

## Consumer

Who uses the destination and why it matters now.

## Decisions So Far

Settled decisions only; detailed evidence belongs in child tickets.

## Not Yet Specified

Fog that is not ready for implementation tickets.

## Open Questions Blocking Slicing

Questions that must become one-ticket-per-session child issues.

## Out Of Scope

Nearby work this map must not absorb.

## Frontier

Ready child issue IDs visible through `bd ready`:

Only list unblocked child issues or the dependency that will unblock them.

## Native Dependency Edges

Required `bd dep` edges:

Map blockers as native Beads deps; do not rely on this section alone.
...
## 22. .agents/skills/source-to-decision/references/knowledge-gate.md
...
## 23. .agents/skills/source-to-decision/references/source-usefulness.md
...
## 24. .agents/skills/source-to-decision/SKILL.md
...
## 25. .agents/skills/target-repo-testing/agents/openai.yaml
...
## 26. .agents/skills/target-repo-testing/references/owner-file-read-model.md
...
## 27. .agents/skills/target-repo-testing/SKILL.md
...
For `krn init --repo`, owner-file capture, or activation-read-model trials, load
